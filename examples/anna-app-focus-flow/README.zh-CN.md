...
`http://localhost:5180` 启动 harness，自动发现 `executas/focus-session/`
...
`uv run --project executas/focus-session <minted-tool-id>` 懒启动 executa
进程。**如果该子进程立刻退出**，harness 会在 RPC 日志里报
`tool_failed: executa process exited` —— 此时去
`cd executas/focus-session && uv sync` 跑一遍，可以看到真正的依赖
解析错误。
