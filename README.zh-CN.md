...
│   └── anna-app-focus-flow/             # ⭐ 完整 Anna App 示例（UI bundle + skill + tool 插件）
│                                        #     Tool 插件提供三种语言实现：
│                                        #     focus-session-{python,node,go}；
│                                        #     通过 `executa.json` 或 `--executa` CLI flag 切换。
...
### 本地直接运行任意插件 — `anna-app executa dev`

[`anna-app-cli`](https://www.npmjs.com/package/@anna-ai/app-cli) 提供独立
运行器，能在隔离环境中拉起单个 Executa 插件 —— 效果类似于
`anna-app dev` 之于完整 Anna App，**无需** matrix-nexus、Dashboard 或
UI bundle。

```bash
cd examples/python/basic-tool
anna-app executa dev                        # 交互式 REPL
anna-app executa dev --describe             # 一次性：打印 MANIFEST
